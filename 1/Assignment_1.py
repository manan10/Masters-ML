...
def train_model(Phi, y):
    ...
    return np.dot(np.dot(np.linalg.inv(np.dot(transpose, Phi)), transpose), y)    
...
def evaluate_model(Phi, y, w):
    ...
    for x in range(len(Phi)):
        sigma += (y[x] - np.dot(np.transpose(w), Phi[x]))**2
        
    return sigma/len(Phi)
...
# From above graph we can say that both the validation and test errors are
# lower than 21. It shows that when the value of d increases the error values
# gradually decreases and after d=21 the escalation is observed.
...
def train_ridge_model(Phi, y, lam):
    transpose = np.transpose(Phi)
    return np.dot(np.dot(np.linalg.inv((np.dot(transpose, Phi)) + lam*(np.identity(len(Phi)))), transpose), y)
