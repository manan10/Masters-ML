...
def partition(x):
    ...
    for i in range(len(x)):
        if x[i] in vectorDict:
            vectorDict[x[i]].append(i)
        else:
            vectorDict[x[i]] = [i] 
...
def mutual_information(x, y):
    ...
    gainY = entropy(partition(y))
    
    gainX = 0     
    for val in partition(x).values():
        tempY = [y[i] for i in val]
        gainX += ((len(tempY)/len(x)) * entropy(partition(tempY))) 
...
def id3(x, y, attribute_value_pairs=None, depth=0, max_depth=5):
    ...
    * Leaf nodes are (majority) class labels
    ...
    attribute_value_pairs.remove((best_attr, value)) 
    indexes_if_true = partitioned[value] 
    indexes_if_false = [i for i in range(len(x[:,0])) if i not in indexes_if_true]
...
def predict_example(x, tree):
    ...
    if x[node[0][0]] == node[0][1]:
    	return predict_example(x,tree[node[1]])
    else:
    	return predict_example(x,tree[node[0]])
...
def generateAttributeValuePairs(x):
    """
        Function to generate initial attribute-value-pairs
    """
    avpairs = []
